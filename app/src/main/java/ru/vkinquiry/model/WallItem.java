...
public class WallItem extends RealmObject {
    ...
    public boolean haveSharedRepost() {                                                             // метод проверки - содержит ли запись репост
        return copyHistory.size() > 0;
    }

    public WallItem getSharedRepost() {                                                             // получение этого репоста
        if (haveSharedRepost()) {
